...
const TransactionForm = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      const valuesWithEST = {
        ...values,
        date: dayjs(values.date).tz("America/New_York").format(), // Convert date to EST
      };

      try {
        await mutateAsync(valuesWithEST);
        formik.resetForm();
      } catch (e) {
        console.error(e);
      }
    ...
    >
      ...
      {isAddTranErr && (
        <AlertMessage
          type="error"
          message={
            transErr?.response?.data?.message || "Failed to add transaction."
          }
        />
      )}
      {isSuccess && (
